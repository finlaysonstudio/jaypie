 ...
 */
export function random(defaultSeed?: string | number): RandomFunction {
  // Convert seed to string if it's a number
  const seedString =
    defaultSeed !== undefined ? String(defaultSeed) : undefined;

  // Initialize default seeded RNG
  const defaultRng = RandomLib.clone(seedString);
