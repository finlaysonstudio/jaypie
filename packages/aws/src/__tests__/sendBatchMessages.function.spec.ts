...
describe("Send Batch Messages Function", () => {
  ...
  describe("Error Cases", () => {
    it("Throws if messages is not an array", async () => {
      await expect(
        sendBatchMessages({
          messages: "MOCK_MESSAGES" as unknown as unknown[],
        }),
      ).rejects.toThrow();
    });
  ...
  describe("Features", () => {
    it("Chunks messages into batches of max 10", async () => {
      ...
      expect(
        (SendMessageBatchCommand as Mock).mock.calls[0][0].Entries,
      ).toBeArrayOfSize(10);
      expect(
        (SendMessageBatchCommand as Mock).mock.calls[1][0].Entries,
      ).toBeArrayOfSize(10);
      expect(
        (SendMessageBatchCommand as Mock).mock.calls[2][0].Entries,
      ).toBeArrayOfSize(6);
    });
    it("Sends a FIFO message if queueUrl ends in `.fifo`", async () => {
      ...
      const call = (SendMessageBatchCommand as Mock).mock.calls[0][0];
