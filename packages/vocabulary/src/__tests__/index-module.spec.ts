/**
 * Tests for @jaypie/vocabulary index module
 */
...
describe("generateIndexName", () => {
  ...
  it("capitalizes each field", () => {
    expect(generateIndexName(["ou", "model", "alias"])).toBe("indexOuModelAlias");
  });
});
...
describe("calculateIndexSuffix", () => {
  ...
  it("returns #archived#deleted for both", () => {
    const entity = { model: "record", archivedAt: new Date(), deletedAt: new Date() };
