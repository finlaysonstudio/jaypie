...
export async function initializeClient({
  ...
  const resolvedApiKey =
    apiKey || (await getEnvSecret(PROVIDER.MISTRAL.API_KEY));
  ...
  const client = new MistralClient({ apiKey: resolvedApiKey });
  logger.trace("Initialized Mistral client");
  return client;
}

// Get default model from environment or constants
export function getDefaultModel(): string {
  return process.env.MISTRAL_MODEL || PROVIDER.MISTRAL.DEFAULT;
}
