...
describe("weather tool", () => {
  ...
  describe("Message Functionality", () => {
    it("returns correct message when no location is provided", () => {
      const message =
        typeof weather.message === "function"
          ? weather.message()
          : weather.message;
      expect(message).toBe(
        "Getting weather for Evanston, IL (42.051554533384866, -87.6759911441785)",
      );
    });
    ...
    it("returns correct message when location is provided", () => {
      const message =
        typeof weather.message === "function"
          ? weather.message({ location: "Chicago, IL" })
          : weather.message;
      expect(message).toBe(
        "Getting weather for Chicago, IL (42.051554533384866, -87.6759911441785)",
      );
    });
