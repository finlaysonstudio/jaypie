...
provider's reason on `ParsedResponse.incompleteReason` (OpenAI and xAI read
`status: "incomplete"` plus `incomplete_details.reason`, such as
`max_output_tokens` or `content_filter`; the stream reads the
`response.incomplete` event), and the loop settles with the partial text as
